...
import { Gamepad2, Sparkles, Code2, Palette, Calendar, Users, ChevronRight, Heart, Zap, Trophy, Star } from "lucide-react";
...
export function Footer() {
  ...
  const mainLinks = [{ label: "Home", href: "/", icon: <Zap className="w-4 h-4" /> }, { label: "About", href: "/about", icon: <Users className="w-4 h-4" /> }, { label: "Events", href: "/events", icon: <Calendar className="w-4 h-4" /> }, { label: "Game Showcase", href: "/game-showcase", icon: <Gamepad2 className="w-4 h-4" /> }, { label: "Art Showcase", href: "/art-showcase", icon: <Palette className="w-4 h-4" /> }];
  const socialLinks = [
  ...
  const stats = [{ label: "Active Members", value: "150+", icon: <Users className="w-4 h-4" /> }, { label: "Games Created", value: "50+", icon: <Gamepad2 className="w-4 h-4" /> }, { label: "Events Hosted", value: "25+", icon: <Trophy className="w-4 h-4" /> }];
    ...
    <div className="relative z-10 border-t border-purple-500/20">
      <div className="container mx-auto px-4 py-12 sm:px-6 lg:px-8">
        <div className="grid grid-cols-1 lg:grid-cols-4 gap-8 mb-12">
          <div className="lg:col-span-1 space-y-4">
            <div className="flex items-center gap-3 group">
              <motion.div className="relative" whileHover={{ scale: 1.1 }} transition={{ type: "spring", stiffness: 400, damping: 17 }}>
                <motion.div className="w-14 h-14 rounded-2xl bg-white flex items-center justify-center shadow-2xl shadow-purple-500/25 overflow-hidden p-1" whileHover={{ boxShadow: "0 0 40px rgba(139, 92, 246, 0.5)" }} transition={{ duration: 0.3 }}>
                  <Image src="/logo.png" alt="UWA Game Development Logo" width={48} height={48} className="object-contain" priority unoptimized />
                </motion.div>
                <motion.div className="absolute -bottom-1 -right-1" animate={{ scale: [1, 1.2, 1], rotate: [0, 180, 360] }} transition={{ duration: 3, repeat: Infinity, ease: "easeInOut" }}><Sparkles className="w-4 h-4 text-yellow-400" /></motion.div>
              </motion.div>
              <div>
                <h3 className="text-xl font-bold text-white bg-gradient-to-r from-purple-400 to-pink-400 bg-clip-text text-transparent">Game Development</h3>
                <p className="text-xs text-gray-400">Create • Play • Inspire</p>
              </div>
            </div>
            <p className="text-sm text-gray-300/80 leading-relaxed">Building the next generation of game developers at UWA game development club</p>
          ...
          <div className="lg:col-span-1 space-y-3">
            <h4 className="text-sm font-semibold text-white uppercase tracking-wider flex items-center gap-2"><Zap className="w-4 h-4 text-purple-400" />Quick Links</h4>
            <ul className="space-y-2">{quickLinks.map((link) => <li key={link.label}><Link href={link.href} className="text-gray-400 text-sm hover:text-purple-400 transition-all duration-300 flex items-center gap-2 group" onMouseEnter={() => setIsHovered(link.label)} onMouseLeave={() => setIsHovered(null)}><ChevronRight className={`w-3 h-3 transition-transform duration-300 ${isHovered === link.label ? 'translate-x-1' : ''}`} /><span className="relative">{link.label}{isHovered === link.label && <span className="absolute inset-x-0 -bottom-0.5 h-px bg-gradient-to-r from-purple-400 to-pink-400" />}</span></Link></li>)}</ul>
          </div>
          <div className="lg:col-span-1 space-y-3">
            <h4 className="text-sm font-semibold text-white uppercase tracking-wider flex items-center gap-2"><Code2 className="w-4 h-4 text-purple-400" />Explore</h4>
            <ul className="space-y-2">{mainLinks.map((link) => <li key={link.label}><Link href={link.href} className="text-gray-400 text-sm hover:text-purple-400 transition-all duration-300 flex items-center gap-2 group"><span className="transition-transform duration-300 group-hover:scale-110 text-purple-500/50 group-hover:text-purple-400">{link.icon}</span>{link.label}</Link></li>)}</ul>
          </div>
          <div className="lg:col-span-1 space-y-4">
            <h4 className="text-sm font-semibold text-white uppercase tracking-wider flex items-center gap-2"><Trophy className="w-4 h-4 text-purple-400" />Community Stats</h4>
            <div className="grid grid-cols-1 gap-3">{stats.map((stat, index) => <div key={index} className="flex items-center justify-between p-3 rounded-lg bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 hover:border-purple-500/40 transition-all duration-300"><div className="flex items-center gap-2"><span className="text-purple-400">{stat.icon}</span><span className="text-xs text-gray-400">{stat.label}</span></div><span className="text-sm font-bold text-white">{stat.value}</span></div>)}</div>
            <div className="pt-2"><motion.button className="w-full py-2.5 px-4 rounded-xl bg-gradient-to-r from-purple-600 to-pink-600 text-white text-sm font-semibold flex items-center justify-center gap-2" whileHover={{ scale: 1.02, y: -2, boxShadow: "0 10px 25px rgba(139, 92, 246, 0.25)" }} whileTap={{ scale: 0.98 }} transition={{ type: "spring", stiffness: 400, damping: 17 }}><motion.div animate={{ rotate: [0, 180, 360] }} transition={{ duration: 2, repeat: Infinity, ease: "linear" }}><Star className="w-4 h-4" /></motion.div>Join Newsletter</motion.button></div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row items-center justify-between gap-4">
          <div className="flex items-center gap-2 text-sm text-gray-400"><span>© {new Date().getFullYear()} CFC Game Dev</span><span className="text-purple-500">•</span><span>All rights reserved</span></div>
          <Link href="/constitution" className="group flex items-center gap-2.5 px-4 py-2 rounded-full bg-gradient-to-r from-purple-500/10 to-pink-500/10 border border-purple-500/20 hover:border-purple-500/40 transition-all duration-300 hover:shadow-lg hover:shadow-purple-500/20">
            <span className="text-sm text-gray-300 group-hover:text-white transition-colors">Constitution</span>
          ...
          <div className="flex items-center gap-2 text-sm text-gray-400"><span>Made with</span><motion.div animate={{ scale: [1, 1.2, 1] }} transition={{ duration: 1.5, repeat: Infinity }}><Heart className="w-4 h-4 text-red-500 fill-current" /></motion.div><span>in Perth, UWA</span></div>
        </div>
      </div>
    </div>
  </footer>;
